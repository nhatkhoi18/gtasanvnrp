...
public class SAMP extends GTASA implements CustomKeyboard.InputListener, HeightProvider.HeightListener {
    ...
    private static boolean sampNativeLoaded = false;

    private static synchronized void ensureSampNativeLoaded() {
        if (sampNativeLoaded) return;
        System.loadLibrary("samp");
        sampNativeLoaded = true;
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Load SA-MP only after the base GTA activity has initialized its
        // native runtime and surface lifecycle. This avoids running fixed-offset
        // hooks during GameNative class initialization.
        try {
            ensureSampNativeLoaded();
        } catch (Throwable t) {
            Log.e(TAG, "Failed to load libsamp", t);
            finish();
            return;
        }
